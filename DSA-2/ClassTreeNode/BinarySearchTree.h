...
#include "..\..\DSA-1\classStack\stack.h"
using namespace std;
...
public:
    ...
    /**
        ...
        * по элементам бинарного дерева поиска
        * Реализует обход in-order (левый -> корень -> правый) при помощи стека
        * Итератор не хранит всё дерево сразу.
        * Он использует стек узлов, чтобы пошагово двигаться по дереву.
        */
    class Iterator : public AbstractIterator<T> {
        ::stack<TreeNode<T>*> nodeStack; /// стек узлов
        TreeNode<T>* current;          /// текущий узел, на который указывает итератор

    private:
        /// Внутренняя функция: извлекает следующий элемент из стека
        void moveNext() {
            ...
            // Берём верхний элемент из стека
            current = nodeStack.pop();

            // Переходим в правое поддерево и кладём все его левые потомки в стек
            if (current->right)
    ...
    public:
         ...
         * Если root != nullptr, стек заполняется всеми левыми потомками до самого низа.
         */
        Iterator(TreeNode<T>* root = nullptr) : current(nullptr) {
            pushLeft(root);
            moveNext();
        }

        /// Вспомогательная функция: помещает в стек все левые узлы от данного
        ...
        /// Если стек не пуст — берём верхний узел, добавляем в стек левые потомки его правого поддерева.
        Iterator& operator++() override {
            moveNext();
            return *this;
        }

        /// Разыменование итератора — доступ к значению текущего узла
        /// @throws out_of_range — если итератор указывает за пределы дерева
        T& operator*() const override {
            if (!current)
                throw out_of_range("Разыменование итератора за пределами дерева");
            return current->value;
        }

        /// Сравнение итераторов на неравенство
        /// Итераторы равны, если оба указывают на один и тот же узел.
        bool operator!=(const AbstractIterator<T>& other) const override {
            const Iterator* it = dynamic_cast<const Iterator*>(&other);
            if (!it)
                return true; // разные типы итераторов точно не равны
            return this->current != it->current; // отличаются узлы, на которые указывают итераторы
        }
    };

    /// Возвращает итератор, указывающий на первый элемент (минимальный)
    /// begin() вызывает конструктор итератора
    Iterator begin() const {
        return Iterator(root);
    }

    /// Возвращает итератор, указывающий за пределы дерева
    /// это просто итератор с current == nullptr
    Iterator end() const {
